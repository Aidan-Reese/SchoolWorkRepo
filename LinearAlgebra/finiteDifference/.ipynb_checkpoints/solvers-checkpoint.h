...
void GaussSeidel(VDD &A , VD &temp , VD b , int nTemp )
  {
	...
	if ( ++iter > max_iter )
	  {
	    cout << "GaussSeidel: max_iter of " << max_iter << " reached.\n";
	    return;
	  }
	...
	rLOOP
	  {
	    ...
	    temp[r] = newval; //Changed from tempNew to temp, as in Gauss-seidel method you immediatly exchange values and dont save them
  ...
  while ( converged == 0  && ++iter <= max_iter)
    {
      ...
      alpha    = r_dot_r / p_dot_Ap;  //TO-DO


      // Update solution
      
      rowLOOP temp [row] = temp[row] + alpha *  p[row]; //To-do

      // Update residual
      
      rowLOOP rnew [row] = r[row] - temp[row] * Ap[row]; //TO-DO

      // Compute beta

      rnew_dot_rnew = Dot(rnew,rnew,nTemp);
      beta          = -(rnew_dot_rnew/r_dot_r); //To-do

      // Update search direction
      
      rowLOOP p[row] = rnew[row] + beta*p[row]; //to-do
